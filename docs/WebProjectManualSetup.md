# Guía para crear manualmente el proyecto `WebSYJ`
> **Nota:** La solución actual ya viene separada con los proyectos `SYJBD` (app interna), `WebSYJ` (sitio público) y la biblioteca compartida `SYJBD.Core`.
> Usa esta guía solo como referencia si necesitas recrear la estructura manualmente o agregar nuevos frentes.


Esta guía resume los pasos para clonar la parte "sitio público" del proyecto MVC actual en un nuevo proyecto `WebSYJ`, creado manualmente en Visual Studio (o con `dotnet new`). El objetivo es aislar la capa de presentación pública sin tocar la aplicación interna existente.
...
## 2. Crear manualmente el proyecto `WebSYJ`
...
## 3. Configurar `Program.cs` en `WebSYJ`
...
1. Ejecuta `dotnet build SYJBD.sln` para comprobar que los tres proyectos (`SYJBD`, `SYJBD.Core`, `WebSYJ`) compilan.
...
Siguiendo estos pasos podrás crear manualmente `WebSYJ`, copiar únicamente los archivos necesarios y garantizar que las dependencias (NuGet y servicios) coincidan con la aplicación original sin romperla.
